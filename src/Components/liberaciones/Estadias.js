...
const StyledTh = styled.th`
  background-color: #9e2343;  // por veda 
  color: white;
  padding: 10px;
  font-size: 20px;
`;
...
const StyledButton = styled.button`
  background-color: ${(props) => (props.validar ? '#9e2343' : '#bc955b')}; //por veda '' : ''
  color: white;
  cursor: pointer;
  border-radius: 15px;
`;
 ...
 //   console.log(solicitudId)
  ...
  const fetchDataTabla = async () => {
    ...
    //    console.log(responseData);
        ...
        const solicitudesFiltradas = responseData.solicitudes.filter(solicitud => solicitud.tipo === 'Estadía' && solicitud.universidad === 'SEMSyS');
...
// Cargar Montserrat Regular
 ...
 const textoPDF = `
                                                  ...
                                                  Carta de Terminción de Estadía


                                                                                                                                                   T-ES-00${datosSolicitud}`;
